    ...
    "cells": [
        ...
        },
        {
            ...
            "source": [
                ...
                "!nvidia-smi\n",
                "!pip install -q onnx onnxruntime onnxscript tqdm matplotlib pillow"
            ]
        },
        {
            "cell_type": "code",
        ...
        },
        {
            ...
            "outputs": [],
            "source": [
                "# Step 3: ResNet-50 + CBAM Neural Network Architecture (PlantPAD Pre-Trained)\n",
                "import os, torch\n",
                "import torch.nn as nn\n",
                "from torchvision import models\n",
                "\n",
                "# Download official PlantPAD / PDDD-PreTrain (400k plant disease images) weights from Zenodo\n",
                "if not os.path.exists('ResNet50-Plant-model-80.pth'):\n",
                "    print('Downloading PlantPAD / PDDD-PreTrain weights (90.9 MB) from Zenodo...')\n",
                "    !curl -L 'https://zenodo.org/api/records/7856293/files/ResNet50-Plant-model-80.pth/content' -o ResNet50-Plant-model-80.pth\n",
                "\n",
                ...
                "        )\n",
                "        # Load PlantPAD plant-specific weights if available\n",
                "        if os.path.exists('ResNet50-Plant-model-80.pth'):\n",
                "            print('🌿 Loading PlantPAD / PDDD-PreTrain (400,000 plant disease images) weights...')\n",
                "            sd = torch.load('ResNet50-Plant-model-80.pth', map_location='cpu')\n",
                "            sd.pop('fc.weight', None)\n",
                "            sd.pop('fc.bias', None)\n",
                "            self.load_state_dict(sd, strict=False)\n",
                "            print('✅ PlantPAD pre-trained weights loaded into ResNet-50 backbone!')\n",
                "        # Freeze early layers, fine-tune layer4 + layer3 + CBAM + classifier\n",
                ...
                "            p.requires_grad = True\n",
                "        for b in list(self.layer3.children())[-2:]:\n",
                ...
                "\n",
                "print('Architecture defined with PlantPAD Pre-Trained Backbone + CBAM Attention!')"
            ]
        },
        {
            "cell_type": "code",
        ...
        },
        {
            ...
            "source": [
                ...
                "    {'params': model.cbam.parameters(), 'lr': 2e-4},\n",
...
target_path = r"c:\Dehati AI\backend\ai_pipeline\DehatiAI_ResNet50_CBAM_GPU_Training.ipynb"
