...
const DM_FACTORS = {
  ...
  'بکری / بھیڑ':   0.035,
  'گھوڑا / خچر':  0.025,
  'مرغی (1 پرندہ)': null // special case
};
    ...
    <div className="form-group">
      <div>
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: '.4rem', flexDirection: 'row-reverse' }}>
          {Object.keys(DM_FACTORS).filter(a => DM_FACTORS[a] !== null).map(a => (
            <button
              key={a}
      ...
      <div>
        <label className="input-label">جانوروں کی تعداد</label>
      ...
      <button className="btn btn-primary btn-full" onClick={calculate} disabled={!animal || !weight || !count} id="feed-calc-btn">
        ✓ خوراک حساب کریں
      </button>
      {result && (
